...
def train_epoch(epoch, loader, iters, lora_params, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels, loss_mask, attention_mask) in enumerate(loader, start_step + 1):
        ...
        # 进入 with 块：
        # 模型参数还是 float32 存储
        # 但前向计算时自动转成 bfloat16 执行
        # bfloat16 的好处：
        #   显存占用减半：float32(4bytes) → bfloat16(2bytes)
        #   计算速度更快：GPU 对半精度有硬件加速
        # bfloat16 的代价：
        #   精度略低，但 bfloat16 数值范围和 float32 一样
        #   不容易溢出，比 float16 更稳定
...
if __name__ == "__main__":
    ...
    # ========== 8. DDP包装模型 ==========
    # 📚 分布式数据并行
    # DistributedDataParallel: PyTorch 的 DDP 实现
